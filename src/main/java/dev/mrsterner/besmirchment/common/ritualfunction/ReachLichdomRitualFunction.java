...
public class ReachLichdomRitualFunction extends RitualFunction {
    ...
    public boolean isValid(ServerWorld world, BlockPos pos, Inventory inventory) {
        ...
        if (BSMTransformations.isLich(closestPlayer, false)){
            closestPlayer.sendMessage( Text.translatable("message.besmirchment.no_liches"), true);
            return false;
        }else if (!BWComponents.CURSES_COMPONENT.get(closestPlayer).hasCurse(BWCurses.APATHY)){
            closestPlayer.sendMessage(Text.translatable("message.besmirchment.needs_apathy"), true);
            return false;
        }
    ...
    public void start(ServerWorld world, BlockPos glyphPos, BlockPos effectivePos, Inventory inventory, boolean catFamiliar) {
        ...
        if (!BSMTransformations.isLich(closestPlayer, false) && (BWComponents.CURSES_COMPONENT.get(closestPlayer).hasCurse(BWCurses.APATHY) || !BWConfig.enableCurses)){
